...
pairs(a,
      col= c("red", "blue")[as.integer(a$Producto)], 
...
```{r}
# Supongamos que tienes un dataframe llamado 'dades' con las variables de interés

# Centrar los datos (opcional para covarianzas)
medias <- colMeans(dades[, c("Ventas", "Publicidad", "Satisfaccion_cliente")])  # Calcula la media
datos_centrados <- scale(dades[, c("Ventas", "Publicidad", "Satisfaccion_cliente")], center = TRUE, scale = FALSE)  # Centra los datos

# Calcular la matriz de covarianzas
matriz_covarianzas <- cov(dades[, c("Ventas", "Publicidad", "Satisfaccion_cliente")])

# Calcular la matriz de correlaciones
matriz_correlaciones <- cor(dades[, c("Ventas", "Publicidad", "Satisfaccion_cliente")])
# Altra forma- apuntes
n=length(dades$Producto)
matriu_corr <- matriz_covarianzas*(n-1)/n
matriu_corr
